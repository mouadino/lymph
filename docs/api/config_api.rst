...
.. class:: Configuration(values=None)
    ...
    .. method:: create_instance(key, default_class=None, args=(), **kwargs)

        :param key: dotted config path (e.g. ``"container.rpc"``)
        :param default_class: class object or fully qualified name of a class
        :param args: positional arguments to be passed to the factory.
    ...
    .. method:: get_instance(key, default_class, args=(), **kwargs)

        Like ``create_instance()``, but only creates a single instance for each
        key.

    .. method:: get_dependency(key, args=(), **kwargs)

        Get an instance from dependencies.

        Give the following config file
        ...
        you can get the dependency by doing

        .. code-block:: python

            config = Configuration()
            config.load(...)
            config.get_dependency('some')
